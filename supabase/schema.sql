-- ═══════════════════════════════════════════════════════════════════
-- Jobkar — Supabase schema (v11: website + login-gated desktop dashboard)
...
-- Trigram matching for job search (ILIKE acceleration + did-you-mean).
...
create table public.profiles (
  ...
  upi_id text check (upi_id ~ '^[a-z0-9._-]{2,}@[a-z]{2,}$'),  -- optional UPI payout rail
...
create table public.job_listings (
  ...
  -- Numeric facets parsed from salary_range/experience by the trigger below
  -- (search_jobs filters on these; null = unparseable → matches every bucket).
  salary_monthly_min int,
  salary_monthly_max int,
  exp_min_months int,
  exp_max_months int,
...
-- Trigram indexes for search_jobs token ILIKE.
create index job_listings_title_trgm_idx
  on public.job_listings using gin (title gin_trgm_ops);
create index job_listings_company_trgm_idx
  on public.job_listings using gin (company gin_trgm_ops);
create index job_listings_description_trgm_idx
...
create table public.withdrawal_requests (
  ...
  user_id uuid not null references public.profiles(id),
  amount numeric not null check (amount > 0),           -- rupees
  payout_method text not null default 'bank' check (payout_method in ('bank', 'upi')),
  bank_holder_name text,
  bank_account_number text,
  bank_ifsc text,
  upi_id text,                                           -- snapshot when payout_method='upi'
...
-- ─── Search facet parsing ───────────────────────────────────────────
-- Ports lib/jobsFilters.ts parseSalaryRange into SQL (monthly; LPA ÷12;
-- "20k" ×1000; first number before LPA wins). Null-safe: clears facets when
-- the source text is null/empty. Unparseable → null → matches every bucket.
...
-- UPI payout rail: separate RPC (no second update_own_profile overload —
-- PostgREST ambiguity). Null/blank clears the UPI ID.
...
-- ─── Smart Search RPCs (service-role only) ──────────────────────────
-- search_jobs: AND/OR text matching + structured filters + facets, read from
-- the public_jobs view (safe columns; its WHERE + column list stay the
-- authorization). SECURITY DEFINER + service_role-only grant; the website
-- route redacts premium fields per row before anything reaches a browser.
...
-- suggest_job_query: one trigram-similar approved-title word ("did you mean").
...
-- Smart Search RPCs: service-role ONLY (the /api/jobs route is the sole caller).
revoke execute on function public.search_jobs(text[], text[], text, boolean, int, int, int, int, int, text, int, int) from public, anon, authenticated;
grant  execute on function public.search_jobs(text[], text[], text, boolean, int, int, int, int, int, text, int, int) to service_role;
...
-- ─── upi_id added 2026-09-08, see live-fixes/2026-09-08-upi-payouts.sql) ──
...
grant select (
  ...
  terms_accepted_at, created_at, upi_id
